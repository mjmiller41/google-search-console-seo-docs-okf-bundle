---
type: Reference
title: Remove images hosted on your site from search results
description: A robots.txt file can prevent images from appearing in search results. Discover how to remove and hide images on your site from Google Search.
resource: https://developers.google.com/search/docs/crawling-indexing/prevent-images-on-your-page
tags:
- google-search
- documentation
- crawling-indexing
...
sources:
- id: google-prevent-images-on-your-page
  resource: https://developers.google.com/search/docs/crawling-indexing/prevent-images-on-your-page
  title: Remove images hosted on your site from search results
...
> Mirrored from the official Google Search Central documentation at [https://developers.google.com/search/docs/crawling-indexing/prevent-images-on-your-page](https://developers.google.com/search/docs/crawling-indexing/prevent-images-on-your-page). Page content, including any embedded figures, is authored by Google and was last updated by Google on 2025-12-10.[^google-prevent-images-on-your-page]
...
To quickly remove images hosted on your site from Google's search results, use the [Removals tool](https://support.google.com/webmasters/answer/9689846#zippy=%2Cimage-url). Keep in mind that unless you also remove the images from your site or otherwise block the images as described in the [non-emergency image removal section](https://developers.google.com/search/docs/crawling-indexing/prevent-images-on-your-page/#non-emergency-image-removal), the images may resurface in Google's search results once the removal request expires.

## For non-emergency image removal

There are two ways to remove images from your site from Google's search results:

- [robots.txt disallow rules](https://developers.google.com/search/docs/crawling-indexing/prevent-images-on-your-page/#robotstxt)
- [The `noindex` `X-Robots-Tag` HTTP header](https://developers.google.com/search/docs/crawling-indexing/prevent-images-on-your-page/#noindex)
...
To prevent images from your site appearing in Google's search results, add a [robots.txt](/crawling-indexing/robots/intro.md) file to the root of the site that hosts the image, for example `https://yoursite.example.com/robots.txt`. While it takes longer to remove an image from Google's search results using robots.txt rules than it does to use the Removals tool, it gives you more flexibility and control through the use of wildcards or subpath blocking. It also applies to all search engines, whereas the Removals tool only applies to Google.
...
Rules may include [special characters](https://developers.google.com/search/docs/crawling-indexing/robots/robots_txt#url-matching-based-on-path-values) for more flexibility and control. Specifically, the `*` character matches any sequence of characters which lets you to match multiple image paths with one rule.
...
Alternatively, you can remove images hosted on your site from Google's search results by adding the `noindex` `X-Robots-Tag` to the HTTP response headers of the images you want to remove. In this case you must allow crawling the image URLs in order for Googlebot to be able to extract the `noindex` rule. To implement the `noindex` `X-Robots-Tag` HTTP response header, [follow our documentation about `noindex`](/crawling-indexing/robots-meta-tag.md).
...
[^google-prevent-images-on-your-page]: Google Search Central (2025). "Remove images hosted on your site from search results". *Google for Developers*. https://developers.google.com/search/docs/crawling-indexing/prevent-images-on-your-page. Retrieved 2026-09-01.
