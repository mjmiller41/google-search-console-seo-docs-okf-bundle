...
description: Google supports several sitemap formats. Follow this guide to learn about formats, how to build a sitemap, and how to submit a sitemap to Google.
...
> Mirrored from the official Google Search Central documentation at [https://developers.google.com/search/docs/crawling-indexing/sitemaps/build-sitemap](https://developers.google.com/search/docs/crawling-indexing/sitemaps/build-sitemap). Page content, including any embedded figures, is authored by Google and was last updated by Google on 2026-07-08.[^google-build-sitemap]

This page describes how to build a sitemap and make it available to Google. If you're new to sitemaps, [read our introduction first](/crawling-indexing/sitemaps/overview.md).
...
<tr>
<td>
<a href="https://developers.google.com/search/docs/crawling-indexing/sitemaps/build-sitemap/#xml">XML sitemap</a>
</td>
<td>
<p>
        ...
        <a href="/crawling-indexing/sitemaps/image-sitemaps.md">images</a>,
        <a href="/crawling-indexing/sitemaps/video-sitemaps.md">video</a>, and
        <a href="/crawling-indexing/sitemaps/news-sitemap.md">news</a> content, as well
        as the
        <a href="/specialty/international/localized-versions.md">localized versions</a>
        of your pages.
      </p>
<table>
<tr>
...
<tr>
<td><a href="https://developers.google.com/search/docs/crawling-indexing/sitemaps/build-sitemap/#rss">RSS, mRSS, and Atom 1.0</a></td>
<td>
<p>
...
<li>
                Besides
                <a href="/crawling-indexing/indexable-file-types.md">HTML and other indexable textual content</a>,
...
<tr>
<td><a href="https://developers.google.com/search/docs/crawling-indexing/sitemaps/build-sitemap/#text">Text sitemap</a></td>
<td>
<p>
...
Include the URLs in your sitemap that you want to see in Google's search results. Google generally shows the [canonical URLs](/crawling-indexing/consolidate-duplicate-urls.md) in its search results, which you can influence with sitemaps. If you have different URLs for mobile and desktop versions of a page, we recommend pointing to only one version in a sitemap. However, if you want to point to both URLs, [annotate](/crawling-indexing/mobile/mobile-sites-mobile-first-indexing.md) your URLs to indicate the desktop and mobile versions.

For a complete list of best practices, check out the [sitemaps protocol](https://www.sitemaps.org/protocol.html).

## XML sitemap

The XML sitemap format is the most versatile of the supported formats. Using the Google supported sitemap extensions, you can also provide additional information about your [images](/crawling-indexing/sitemaps/image-sitemaps.md), [video](/crawling-indexing/sitemaps/video-sitemaps.md), and [news](/crawling-indexing/sitemaps/news-sitemap.md) content, as well as the [localized versions](/specialty/international/localized-versions.md) of your pages.
...
- You can use an [mRSS (media RSS) feed](/crawling-indexing/sitemaps/video-sitemaps.md) to provide Google details about video content on your site.
- As with all XML files, all tag values must be [entity escaped](https://www.sitemaps.org/protocol.html#escaping).
...
When creating a sitemap, you're telling search engines about which URLs you prefer to show in search results. These are the [canonical URLs](/crawling-indexing/consolidate-duplicate-urls.md). If you have the same content accessible under different URLs, choose the URL you prefer and include that in the sitemap instead of all URLs that lead to the same content.
...
You don't have to worry about the order of the URLs in your sitemap, it doesn't matter to Google. Note the [size requirements for sitemaps](https://developers.google.com/search/docs/crawling-indexing/sitemaps/build-sitemap/#general-guidelines); if the sitemap becomes too large, you must split it into smaller sitemaps. Learn more about [managing large sitemaps](/crawling-indexing/sitemaps/large-sitemaps.md).
...
- **Insert the following line anywhere in your robots.txt file**, specifying the path to your sitemap. We will find it the next time we crawl your robots.txt file:
  ...
  You can specify multiple sitemap lines, and there's no limit to the number of sitemaps you can include in your robots.txt file.
...
[^google-build-sitemap]: Google Search Central (2026). "Build and submit a sitemap". *Google for Developers*. https://developers.google.com/search/docs/crawling-indexing/sitemaps/build-sitemap. Retrieved 2026-09-01.
