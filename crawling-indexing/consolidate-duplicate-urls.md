...
description: When a site has duplicate content, Google chooses the canonical URL. Learn more about canonical URLs and how to consolidate duplicate URLs.
...
> Mirrored from the official Google Search Central documentation at [https://developers.google.com/search/docs/crawling-indexing/consolidate-duplicate-urls](https://developers.google.com/search/docs/crawling-indexing/consolidate-duplicate-urls). Page content, including any embedded figures, is authored by Google and was last updated by Google on 2026-07-10.[^google-consolidate-duplicate-urls]

To specify a [canonical URL](/crawling-indexing/canonicalization.md) for duplicate or very similar pages to Google Search, you can indicate your preference using a number of methods. These are, in order of how strongly they can influence canonicalization:
...
- **Don't** specify a URL fragment as canonical, as [Google generally doesn't support URL fragments](/crawling-indexing/url-structure.md).
- **Do** include a `rel="canonical"` link on the canonical page itself (also known as a self-referential canonical).
- **We don't recommend** using [`noindex`](/crawling-indexing/block-indexing.md) to prevent selection of a canonical page within a single site, because it will completely block the page from Search. `rel="canonical"` `link` annotations are the preferred solution.
- If you're using [`hreflang` elements](/specialty/international/localized-versions.md), make sure to specify a canonical page in the same language, or the best possible substitute language if a canonical page doesn't exist for the same language.
...
<tr>
<td>
<a href="https://developers.google.com/search/docs/crawling-indexing/consolidate-duplicate-urls/#rel-canonical-link-method"><code>rel="canonical" link</code> element</a>
</td>
<td>
<p>
        Add a <code>&lt;link&gt;</code> element in the code for all duplicate pages,
...
<li>
                ...
                you can use the <code>rel="canonical"</code> HTTP header.
              </li>
</ul>
</td>
</tr>
...
<tr>
<td>
<a href="https://developers.google.com/search/docs/crawling-indexing/consolidate-duplicate-urls/#rel-canonical-header-method"><code>rel="canonical"</code> HTTP header</a>
</td>
<td>
<p>Send a <code>rel="canonical"</code> header in your page response.</p>
<table>
<tr>
<td>
<b>Pros:</b>
...
<tr>
<td><a href="https://developers.google.com/search/docs/crawling-indexing/consolidate-duplicate-urls/#sitemap-method">Sitemap</a></td>
<td>
...
<p><b>Pros:</b></p>
<ul>
<li>Simple to implement and maintain, especially on large sites.</li>
</ul>
</td>
<td>
<p><b>Cons:</b></p>
...
<li>
                Less powerful signal to Google than the <code>rel="canonical"</code>
                mapping technique.
              </li>
</ul>
</td>
...
<tr>
<td><a href="https://developers.google.com/search/docs/crawling-indexing/consolidate-duplicate-urls/#redirects-method">Redirects</a></td>
<td>
...
<tr>
<td><a href="/crawling-indexing/amp/overview.md">AMP variant</a></td>
<td>
...
3.  Add any [`hreflang`](/specialty/international/localized-versions.md) or other elements that are appropriate for the page.
...
The `rel="canonical"` `link element` is only accepted if it appears in the `<head>` section of the HTML, so make sure at least the [`<head>` section is valid HTML](/crawling-indexing/valid-page-metadata.md).

If you use JavaScript to add the `rel="canonical"` `link` element, make sure to [inject the canonical link element properly](/crawling-indexing/javascript/javascript-seo-basics.md).
...
Pick a canonical URL for each of your pages and submit them in a [sitemap](/crawling-indexing/sitemaps/overview.md). All pages listed in a sitemap are suggested as canonicals; Google will decide which pages (if any) are duplicates, based on similarity of content.
...
Use this method when you want to get rid of existing duplicate pages. All [permanent redirection methods](/crawling-indexing/301-redirects.md) have the same effect on Google Search, however the time it takes for search engines to notice the different redirect methods may differ.
...
- Don't include the HTTP version of your pages in your sitemap or [`hreflang` annotations](/specialty/international/localized-versions.md) rather than the HTTPS version.
...
Read more about [troubleshooting and fixing canonicalization issues](/crawling-indexing/canonicalization-troubleshooting.md).

# References & Citations

[^google-consolidate-duplicate-urls]: Google Search Central (2026). "How to specify a canonical URL with rel="canonical" and other methods". *Google for Developers*. https://developers.google.com/search/docs/crawling-indexing/consolidate-duplicate-urls. Retrieved 2026-09-01.
