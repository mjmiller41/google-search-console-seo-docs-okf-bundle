...
description: Learn how to troubleshoot crawling errors.
resource: https://developers.google.com/search/docs/crawling-indexing/troubleshoot-crawling-errors
tags:
- google-search
- documentation
- crawling-indexing
...
sources:
- id: google-troubleshoot-crawling-errors
  resource: https://developers.google.com/search/docs/crawling-indexing/troubleshoot-crawling-errors
  title: Troubleshoot Google Search crawling errors
...
> Mirrored from the official Google Search Central documentation at [https://developers.google.com/search/docs/crawling-indexing/troubleshoot-crawling-errors](https://developers.google.com/search/docs/crawling-indexing/troubleshoot-crawling-errors). Page content, including any embedded figures, is authored by Google and was last updated by Google on 2025-12-18.[^google-troubleshoot-crawling-errors]

Here are the key steps to troubleshooting and fixing Google Search crawling issues for your site:

1.  [See if Googlebot is encountering availability issues on your site](https://developers.google.com/search/docs/crawling-indexing/troubleshoot-crawling-errors/#availability_issues).
2.  [See whether you have pages that aren't being crawled, but should be](https://developers.google.com/search/docs/crawling-indexing/troubleshoot-crawling-errors/#not_crawled_should_be).
3.  [See whether any parts of your site need to be crawled more quickly than they already are.](https://developers.google.com/search/docs/crawling-indexing/troubleshoot-crawling-errors/#updates)
4.  [Improve your site's crawl efficiency.](https://developers.google.com/search/docs/crawling-indexing/troubleshoot-crawling-errors/#improve_crawl_efficiency)
5.  [Handle overcrawling of your site](https://developers.google.com/search/docs/crawling-indexing/troubleshoot-crawling-errors/#emergencies).
...
- **Block pages from crawling if you don't want them to be crawled.** (See *[manage your inventory](https://developers.google.com/crawling/docs/crawl-budget#manage_inventory)*)
- **Increase page loading and rendering speed.** (See *[Improve your site's crawl efficiency](https://developers.google.com/search/docs/crawling-indexing/troubleshoot-crawling-errors/#improve_crawl_efficiency)*)
...
If you are adding pages to your site and they are not being crawled in a reasonable amount of time, either Google doesn't know about them, the content is blocked, your site has reached its maximum serving capacity, or you are [out of crawl budget](https://developers.google.com/crawling/docs/crawl-budget#more-crawl-budget).
...
3.  Review your crawling priorities (a.k.a. use your crawl budget wisely). [Manage your inventory](https://developers.google.com/crawling/docs/crawl-budget#manage_inventory) and [improve your site's crawling efficiency](https://developers.google.com/search/docs/crawling-indexing/troubleshoot-crawling-errors/#improve_crawl_efficiency).
4.  [Check that you're not running out of serving capacity](https://developers.google.com/search/docs/crawling-indexing/troubleshoot-crawling-errors/#availability_issues). Googlebot will scale back its crawling if it detects that your servers are having trouble responding to crawl requests.
...
- Use a [news sitemap](/crawling-indexing/sitemaps/news-sitemap.md) if your site has news content.
- Use the `<lastmod>` tag in sitemaps to indicate when an indexed URL has been updated.
- Use a [crawlable URL structure](/crawling-indexing/url-structure.md) to help Google find your pages.
- Provide standard, [crawlable `<a>` links](/crawling-indexing/links-crawlable.md) to help Google find your pages.
- If your site uses separate HTML for mobile and desktop versions, provide the same set of links on the mobile version as you have on the desktop version. If it's not possible to provide the same set of links on the mobile version, ensure that they're included in a [sitemap](/crawling-indexing/sitemaps/overview.md) file. Google only [indexes the mobile version](/crawling-indexing/mobile/mobile-sites-mobile-first-indexing.md) of pages, and limiting the links shown there can slow down discovery of new pages.
...
- Including URLs in your sitemaps that [you don't want to appear in Search](https://developers.google.com/search/docs/crawling-indexing/troubleshoot-crawling-errors/#hide_urls). This can waste your crawl budget on pages that you don't want indexed.
...
- [Duplicate content](/crawling-indexing/consolidate-duplicate-urls.md): Help Google identify duplicate content to avoid unnecessary crawling.
- [`soft 404` pages](https://developers.google.com/search/docs/crawling-indexing/troubleshoot-crawling-errors/#soft-404-errors): Return a `404` code when a page no longer exists.
...
- [Low quality and spam content](/essentials/spam-policies.md): Good to avoid, obviously.
...
- [The page and content are no longer available.](https://developers.google.com/search/docs/crawling-indexing/troubleshoot-crawling-errors/#pagegone)
- [The page or content is now somewhere else.](https://developers.google.com/search/docs/crawling-indexing/troubleshoot-crawling-errors/#pagemoved)
- [The page and content still exist.](https://developers.google.com/search/docs/crawling-indexing/troubleshoot-crawling-errors/#pageother)
...
If your page has moved or has a clear replacement on your site, return a [`301 (permanent redirect)`](/crawling-indexing/301-redirects.md) to redirect the user. This won't interrupt their browsing experience and it's also a great way to tell search engines about the new location of the page. Use the [URL Inspection tool](https://support.google.com/webmasters/answer/9012289) to verify whether your URL is actually returning the correct code.

##### The page and content still exist

If an otherwise good page was flagged with a `soft 404` error, it's likely it didn't load properly for Googlebot, it was missing critical resources, or it displayed a prominent error message during rendering. Use the [URL Inspection tool](https://support.google.com/webmasters/answer/9012289) to examine the rendered content and the returned HTTP code. If the rendered page is blank, nearly blank, or the content has an error message, it could be that your page references many resources that can't be loaded (images, scripts, and other non-textual elements), which can be interpreted as a `soft 404`. Reasons that resources can't be loaded include blocked resources (blocked by [robots.txt](/crawling-indexing/robots/intro.md)), having too many resources on a page, various server errors, or slow loading or very large resources.
...
[^google-troubleshoot-crawling-errors]: Google Search Central (2025). "Troubleshoot Google Search crawling errors". *Google for Developers*. https://developers.google.com/search/docs/crawling-indexing/troubleshoot-crawling-errors. Retrieved 2026-09-01.
