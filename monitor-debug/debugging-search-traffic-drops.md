...
description: There are several causes for drops in Search traffic. Learn how to investigate what caused a traffic drop and how to fix it.
...
sources:
- id: google-debugging-search-traffic-drops
  resource: https://developers.google.com/search/docs/monitor-debug/debugging-search-traffic-drops
  title: Debugging drops in Google Search traffic
...
> Mirrored from the official Google Search Central documentation at [https://developers.google.com/search/docs/monitor-debug/debugging-search-traffic-drops](https://developers.google.com/search/docs/monitor-debug/debugging-search-traffic-drops). Page content, including any embedded figures, is authored by Google and was last updated by Google on 2025-12-10.[^google-debugging-search-traffic-drops]
...
- **[Small drop in position?](https://developers.google.com/search/docs/monitor-debug/debugging-search-traffic-drops/#small)** For example, dropping from position 2 to 4.
- **[Large drop in position?](https://developers.google.com/search/docs/monitor-debug/debugging-search-traffic-drops/#large)** For example, dropping from position 4 to 29.
...
In cases like this, self-assess your whole website overall (not just individual pages) to make sure it's [helpful, reliable and people-first](/fundamentals/creating-helpful-content.md). If you've made changes to your site, it may take time to see an effect: some changes can take effect in a few days, while others could take several months. For example, it may take months before our systems determine that a site is now producing helpful content in the long term. In general, you'll likely want to wait a few weeks to analyze your site in Search Console again to see if your efforts had a beneficial effect on ranking position.
...
If your site is affected by a [security threat](/monitor-debug/security/overview.md), like malware or phishing, Google may alert users before they reach your site with warnings or interstitial pages, which may decrease Search traffic.
...
Google detects practices that violate Google Search spam policies both through automated systems and, as needed, human review that can result in a [manual action](https://search.google.com/search-console/manual-actions). If your site doesn't comply with the [Spam policies for Google web search](/essentials/spam-policies.md), your content might rank lower in results or not appear in results at all.

If you suspect a drop due to a spam violation, review our [spam policies](/essentials/spam-policies.md) to ensure you're not engaging in spam practices that our automated systems would detect. Also, check the [Manual Actions report](https://search.google.com/search-console/manual-actions) on Search Console to see if any have been issued to your website.
...
If you see a drop after moving and it's not recovering, check the [site move troubleshooting section](/crawling-indexing/site-move-with-url-changes.md) for common mistakes when migrating a site with URL changes.
...
If both impressions and clicks dropped, check the [list of the most common reasons](https://developers.google.com/search/docs/monitor-debug/debugging-search-traffic-drops/#main-causes) that could have caused it. If your impressions remain the same but your clicks drop, you might not be generating the best [page title](/appearance/title-link.md) and [snippet](/appearance/snippet.md) that you could, and so users don't understand the content of your page, or perhaps other sites had a more appealing [rich result](/appearance/structured-data/search-gallery.md).
...
![Date range filter showing Last 16 months in the Performance report in Search Console](https://developers.google.com/static/search/docs/images/performance-report-date-range.png)
...
![Yearly seasonality in the Performance report in Search Console](https://developers.google.com/static/search/docs/images/yearly-seasonality-chart.png)
...
![Date range filter comparison](https://developers.google.com/static/search/docs/images/search-console-date-range-filter-comparison.png)
...
![Comparison mode on Performance report showing drop in traffic](https://developers.google.com/static/search/docs/images/search-traffic-drop-comparison.png)
...
![Search type filter in the Performance report in Search Console](https://developers.google.com/static/search/docs/images/performance-search-type-filter.png)

### Monitor your average position in search results

Click **Average position** above the chart. In general, you shouldn't focus too much on your absolute position. Impressions and clicks are ultimately the measure of success for your site. However, if you do see a dramatic, persistent drop in position, try [self-assessing your content](/fundamentals/creating-helpful-content.md) to gauge if it's helpful and reliable.

![Average position decline in the Performance report in Search Console](https://developers.google.com/static/search/docs/images/search-average-position-decline.png)
...
![Page comparison in the Performance report in Search Console](https://developers.google.com/static/search/docs/images/search-console-performance-page-comparison.png)

## Investigate overall trends in your industry

If you want to go the extra mile, you can [use Google Trends](/monitor-debug/trends-start.md) to understand whether the drop is a wider trend or if it's just happening for your site. These changes can be caused by two main factors:
...
![Trends for turkey, chicken and coffee in Google Trends](https://developers.google.com/static/search/docs/images/google-trends-analysis.png)
...
[^google-debugging-search-traffic-drops]: Google Search Central (2025). "Debugging drops in Google Search traffic". *Google for Developers*. https://developers.google.com/search/docs/monitor-debug/debugging-search-traffic-drops. Retrieved 2026-09-01.
