---
type: Reference
title: Loyalty program (MemberProgram) structured data
description: Learn how to mark up loyalty programs (also known as
resource: https://developers.google.com/search/docs/appearance/structured-data/loyalty-program
tags:
- google-search
- documentation
- appearance
...
> Mirrored from the official Google Search Central documentation at [https://developers.google.com/search/docs/appearance/structured-data/loyalty-program](https://developers.google.com/search/docs/appearance/structured-data/loyalty-program). Page content, including any embedded figures, is authored by Google and was last updated by Google on 2025-12-10.[^google-loyalty-program]

![shopping knowledge panel with loyalty price in search results](https://developers.google.com/static/search/docs/images/loyalty-program.png)
...
1.  Add the [required properties](https://developers.google.com/search/docs/appearance/structured-data/loyalty-program/#structured-data-type-definitions). Based on the format you're using, learn where to [insert structured data on the page](/appearance/structured-data/intro-structured-data.md).
...
2.  Follow the [guidelines](https://developers.google.com/search/docs/appearance/structured-data/loyalty-program/#guidelines).
...
- [Technical guidelines](https://developers.google.com/search/docs/appearance/structured-data/loyalty-program/#technical-guidelines)

### Technical guidelines

- Nest the `MemberProgram` markup under the `Organization` type on the page where you specify the administrative details and policies of your business. Refer to the [Organization markup](/appearance/structured-data/organization.md) documentation for more information.
- To specify the loyalty benefits (such as loyalty prices and points earned) for your individual products, add the `UnitPriceSpecification` markup defined for [merchant listings](/appearance/structured-data/merchant-listing.md). The `MemberProgram` markup you define for your business works together with `validForMemberTier` and `MembershipPointsEarned` structured data to define the loyalty benefits for your customers when buying your products.
...
<td>
<p>Repeated <code><a href="https://schema.org/MemberProgramTier">MemberProgramTier</a></code></p>
<p>Defines a tier under a loyalty program. A loyalty program must have at least one tier.
                See the list of <a href="https://developers.google.com/search/docs/appearance/structured-data/loyalty-program/#memberprogram-tier-properties"><code>MemberProgramTier</code> properties</a>
...
<p>A URL of the web page where a shopper can sign up for this loyalty program.
...
<td>
<p>Repeated <code><a href="https://schema.org/TierBenefitEnumeration">TierBenefitEnumeration</a></code></p>
<p>Benefit for members of this member tier. A member tier can have multiple benefits. The short names without the URL prefix
...
<td>
<p>
<code><a href="https://schema.org/CreditCard">CreditCard</a></code>, or
              <code><a href="https://schema.org/MonetaryAmount">MonetaryAmount</a></code>, or
              <code><a href="https://schema.org/UnitPriceSpecification">UnitPriceSpecification</a></code>, or
              <code><a href="https://schema.org/Text">Text</a></code>
</p>
<p>The requirement to join a member tier. If not specified, anyone can join the tier for free. For a non-free tier, specify
              a value of the type representing the requirement to join the tier.
            </p>
<ul>
<li><code>https://schema.org/CreditCard</code>: Specify the credit card that the user needs to sign up for to join the tier.
                For example:
<pre class="devsite-click-to-copy">  "hasTierRequirement": {
    "@type": "CreditCard",
    "name": "Capital Two cashback rewards platinum card"
  }</pre>
</li>
<li><code>https://schema.org/MonetaryAmount</code>: Specify the minimum amount of spending required to join the tier.
...
<li><code>https://schema.org/UnitPriceSpecification</code>: Specify the periodic fee a consumer needs to pay for membership in the tier.
...
<pre class="devsite-click-to-copy">"hasTierRequirement": "Purchase a share in our coop and volunteer a minimum of 1 day a month to keep operating costs low."</pre>
</li>
</ul>
</td>
</tr>
...
<td>
<p><code><a href="https://schema.org/membershipPointsEarned">QuantitativeValue</a></code></p>
...
<td>
<p><code><a href="https://schema.org/URL">URL</a></code></p>
<p>A URL of the web page where a shopper can sign up for this specific member tier.
...
- Review the [guidelines](https://developers.google.com/search/docs/appearance/structured-data/loyalty-program/#guidelines) again to identify if your content isn't compliant with the guidelines. The problem can be caused by either spammy content or spammy markup usage. However, the issue may not be a syntax issue, and so the Rich Results Test won't be able to identify these issues.
...
[^google-loyalty-program]: Google Search Central (2025). "Loyalty program (MemberProgram) structured data". *Google for Developers*. https://developers.google.com/search/docs/appearance/structured-data/loyalty-program. Retrieved 2026-09-01.
