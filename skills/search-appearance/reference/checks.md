...
| APP-08 | Core Web Vitals: point to field data (Search Console CWV report, PageSpeed Insights, CrUX); flag only obvious static risks, labeled as heuristics | `extract_signals.py` `images.missing_dimensions`, `script_bytes` (render-blocking head heuristic) — label every finding "heuristic, not a CWV measurement" | Same fields on built/template files, same labeling requirement | INFO | `appearance/core-web-vitals.md`, `appearance/page-experience.md` |
